// src/screens/admin/AddResidentScreen.tsx
...
const AddResidentScreen = ({ navigation }: any) => {
  ...
  // 🔥 UPDATE: Added 'floor' to the initial state
  const [formData, setFormData] = useState({ name: '', phone: '', email: '', tower: '', floor: '', flatNo: '', password: '' });
  ...
  const handleRegister = () => {
    // 🔥 UPDATE: Added validation for 'floor'
    if (!formData.name || !formData.phone || !formData.tower || !formData.floor || !formData.flatNo || !formData.password) {
      Alert.alert('Incomplete Details', 'Please fill all the required fields including Tower, Floor, Flat No and Password.');
      return;
    }
    ...
    const formattedData = { 
      ...
      floor: formData.floor.trim(), // Clean whitespace
      role: 'RESIDENT', 
      department: '' 
    };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          {/* 🔥 UPDATE: Added 'floor' to the mapped array */}
            ...
            <View key={field} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: theme.surface, borderRadius: 12, borderWidth: 1, borderColor: theme.border, marginBottom: 16, paddingHorizontal: 16 }}>
              <MaterialCommunityIcons 
                name={
                  ...
                  field === 'floor' ? 'stairs' : // 🔥 Icon for Floor
              ...
              <TextInput
                ...
                placeholder={
                  field === 'tower' ? 'Tower (e.g. A, B)' :
                  field === 'floor' ? 'Floor (e.g. Ground, 1st)' : // 🔥 Placeholder for Floor
                ...
                autoCapitalize={field === 'tower' || field === 'flatNo' ? 'characters' : 'none'}
              />
